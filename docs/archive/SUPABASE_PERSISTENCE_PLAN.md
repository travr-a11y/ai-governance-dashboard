...
| ✅ Ops | Production: set `SUPABASE_URL` + `SUPABASE_ANON_KEY` in Railway → Variables if not already (see `DEPLOYMENT.md`). Local: optional `dashboard-config.json` (gitignored); `npm start` runs `prestart` before `serve`. |
