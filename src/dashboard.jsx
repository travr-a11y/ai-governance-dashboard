  ...
  Tooltip, ResponsiveContainer, Legend, LineChart, Line, CartesianGrid
    ...
    function Module1({
      ...
      const cloudPillText = persistToCloud
        ...
        : supabaseConfigured
          ? "Sign in above to enable cloud upload history"
...
Generated by Frank Group AI Governance Dashboard`);
    ...
    function App() {
      ...
      useEffect(() => {
        const ok = supabaseClient && authSession?.user && isAllowedDashboardEmail(authSession.user.email);
        if (!ok) {
          setPeriodsList([]);
          setTrendFlatRows([]);
          return undefined;
        ...
        return () => { cancelled = true; };
      }, [supabaseClient, authSession, cloudRefreshKey]);
      ...
      const savePeriodBlock = useMemo(() => (
        supabaseClient && authSession?.user && isAllowedDashboardEmail(authSession.user.email)
          ? {
          ...
          : null
      ), [supabaseClient, authSession, handleSavePeriod, saveBusy, saveOk, saveErr, rawRows]);
      ...
      return React.createElement("div", { style:{ fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", background:"#f1f5f9", minHeight:"100vh", padding:24, color:COLOURS.bodyText } },
        ...
        React.createElement("div", {
          style:{
            background:"#fff", border:"1px solid #e2e8f0", borderRadius:8, padding:"14px 20px", marginBottom:24,
          ...
          supabaseClient && authSession?.user && isAllowedDashboardEmail(authSession.user.email) && periodsList.length > 0 && React.createElement("div", { style:{ display:"flex", alignItems:"center", gap:10, marginTop:12, flexWrap:"wrap" } },
        ...
        React.createElement("div", { style:{ display:"flex", flexDirection:"column", gap:20 } },
          activeTab === "admin" && (
            adminUnlocked
              ? React.createElement(React.Fragment, null,
                  supabaseClient && React.createElement(AdminAuthGate, { supabase: supabaseClient, session: authSession }),
