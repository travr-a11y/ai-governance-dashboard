...
import {
  ...
  Tooltip, ResponsiveContainer, Legend, LineChart, Line, CartesianGrid
    ...
    /** Internal deploy only — repository must be private. Paste org key before use. */
    ...
    function aggregateData(rows, audRate, behavior, codeRow) {
      ...
      Object.keys(USERS_MAP).forEach(k => {
        if (!Object.keys(byUser).find(e => e.toLowerCase() === k.toLowerCase())) {
    ...
    function hydrateUserFromPeriodRow(pu, audRate) {
      ...
      const limitKey = Object.keys(SPEND_LIMITS).find(k => k.toLowerCase() === String(pu.email || "").toLowerCase());
      const spendLimit = limitKey !== undefined ? SPEND_LIMITS[limitKey] : null;
    ...
    function AdminAuthGate({ supabase, children }) {
...
Generated by Frank Group AI Governance Dashboard`);
      ...
      const generateWithClaude = async () => {
        const key = (ANTHROPIC_REPORT_API_KEY || "").trim();
        if (!key) {
          setAiError("Report API key is not configured. Set ANTHROPIC_REPORT_API_KEY in the dashboard script (private repository only).");
          return;
        }
        setAiLoading(true);
        setAiError(null);
    ...
    function App() {
      ...
      const handleClearAll = () => { handleClear(); handleClearExport(); };
      ...
      const savePeriodBlock = supabaseClient && authSession?.user && isAllowedDashboardEmail(authSession.user.email)
        ? {
      ...
      return React.createElement("div", { style:{ fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", background:"#f1f5f9", minHeight:"100vh", padding:24, color:COLOURS.bodyText } },
        ...
        React.createElement("div", { style:{ display:"flex", flexDirection:"column", gap:20 } },
          activeTab === "admin" && (
            supabaseClient
              ? React.createElement(AdminAuthGate, { supabase: supabaseClient },
        ...
        React.createElement("div", { style:{ textAlign:"center", color:"#9ca3af", fontSize:11, marginTop:24, paddingBottom:12 } }, "Frank Group AI Governance Dashboard · Phase 2 · Optional Supabase persistence; Module 8 may send aggregated metrics to Anthropic; all other processing is client-side")
      );
    }
