...
// ─── Constants ───────────────────────────────────────────────────────────────
    ...
    /** Sonnet model for Module 8 optional AI narrative (Anthropic Messages API). */
    ...
    function buildReportMetricsPayload(users, metrics, initiatives, settings, dateRange) {
    ...
    const REPORT_CLAUDE_SYSTEM = [
      "You are an AI governance analyst for Frank Group (Frank Advisory and Frank Law), Australia.",
      "Write a plain-text executive report for James Frank and the AI Committee using ONLY the JSON metrics provided — do not invent numbers or users.",
      "Australian English. Sections: Executive summary; Adoption and digital fluency; Model mix and cost discipline; AI Committee initiatives (reference targets); User highlights (call out inactive seats); Risks and recommendations; Next steps.",
      "Tone: professional, concise, board-ready. No markdown code fences. Do not include conversation message content (none was supplied).",
    ].join(" ");
...
Generated by Frank Group AI Governance Dashboard`);
      ...
      const generateWithClaude = async () => {
        ...
        if (!key) { setAiError("Paste your Anthropic API key below (session only — not saved)."); return; }
        setAiLoading(true);
        setAiError(null);
        try {
          const payload = buildReportMetricsPayload(users, metrics, initiatives, settings, dateRange);
          ...
          if (!res.ok) {
            setAiError((data && data.error && data.error.message) ? data.error.message : (raw.slice(0, 400) || `HTTP ${res.status}`));
            return;
          }
          const blocks = Array.isArray(data.content) ? data.content : [];
          const text = blocks.filter(c => c.type === "text").map(c => c.text).join("\n").trim();
          if (!text) setAiError("Empty response from Claude.");
          else setReport(text);
        } catch (e) {
          setAiError(String(e.message || e) + " If the browser blocked the request, use Generate report (template) or a server-side proxy.");
        } finally {
          setAiLoading(false);
        }
      };
      ...
      return React.createElement(Card, null,
        ...
        React.createElement("div", { style:{ fontSize:12, color:COLOURS.captionText, marginBottom:12, lineHeight:1.5 } },
          "Generate a report first (template below, or Claude using your API key), then copy, download Word-compatible .doc, plain .txt, or Print / Save as PDF. ",
          React.createElement("strong", null, "Claude option"),
          " sends aggregated metrics to Anthropic — key stays in this tab only; nothing is stored by the dashboard."
        ),
        React.createElement("div", { style:{ background:"#f8fafc", border:"1px solid #e5e7eb", borderRadius:8, padding:"12px 14px", marginBottom:14 } },
          React.createElement("div", { style:{ fontSize:12, fontWeight:600, color:COLOURS.bodyText, marginBottom:8 } }, "Claude narrative (optional)"),
          React.createElement("input", {
            type:"password", autoComplete:"off", placeholder:"Anthropic API key — session only",
            value: anthropicKey, onChange: e => setAnthropicKey(e.target.value),
            style:{ width:"100%", maxWidth:420, border:"1px solid #d1d5db", borderRadius:6, padding:"8px 10px", fontSize:13, marginBottom:8, boxSizing:"border-box" }
          }),
          React.createElement("div", { style:{ display:"flex", flexWrap:"wrap", gap:8, alignItems:"center" } },
            React.createElement("button", {
              ...
              style:{ background:COLOURS.accent, color:"#1a1a1a", border:"none", borderRadius:6, padding:"8px 16px", fontSize:13, fontWeight:700, cursor: aiLoading ? "wait" : "pointer", opacity: aiLoading ? 0.7 : 1 }
            }, aiLoading ? "Calling Claude…" : `Generate with Claude (${ANTHROPIC_REPORT_MODEL})`)
          ),
          React.createElement("div", { style:{ fontSize:11, color:"#9ca3af", marginTop:8 } }, "Requires Anthropic browser access header; if blocked, use template or a Railway proxy with the key server-side.")
        ),
        ...
        React.createElement("div", { style:{ display:"flex", flexWrap:"wrap", gap:10, marginBottom:16 } },
          React.createElement("button", { onClick:generate, style:{ background:COLOURS.advisory, color:"#fff", border:"none", borderRadius:6, padding:"10px 20px", fontSize:13, fontWeight:600, cursor:"pointer" } }, "Generate report (template)"),
